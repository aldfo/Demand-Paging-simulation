...
void simulator(){
    ...
    while(1){
        ...
        list_for_each_entry(cur, &job, list){ 
            ...
            cur_pte_lev1 = (pte*)&pas[cur->pid];
            // level 1 page fault
            int ref_num = cur->ref[ref_cnt];
            if(cur_pte_lev1[ref_num / 8].vflag == PAGE_INVALID){
                // out of range
                if(free_frame >= MAX_REFERENCES){
                    printf("Out of memory!!\n");
                    isbreak = 1;
                    break;
                }
                cur_pte_lev1[ref_num / 8].vflag = PAGE_VALID;
                cur_pte_lev1[ref_num / 8].frame = free_frame++;
                cur->pagefault_cnt++;
            }
            // not level 1 page fault -> 아무 동작 하지 않음. 
            else{}
            
            cur_pte_lev2 = (pte*)&pas[cur_pte_lev1[ref_num / 8].frame];
            // level 2 page fault
            if(cur_pte_lev2[ref_num % 8].vflag == PAGE_INVALID){
                if(free_frame >= MAX_REFERENCES){
                    printf("Out of memory!!\n");
                    isbreak = 1;
                    break;
                }
                cur_pte_lev2[ref_num % 8].vflag = PAGE_VALID;
                cur_pte_lev2[ref_num % 8].frame = free_frame++;
                cur_pte_lev2[ref_num % 8].ref = 1;
                cur->pagefault_cnt++;
                cur->reference_cnt++;
            }
            // not level 2 page fault
            else{
                cur_pte_lev2[ref_num % 8].ref++;
                cur->reference_cnt++;
            }
        }
        if(isbreak) break;
...
void result_print(){
    ...
    list_for_each_entry(cur, &job, list){
        ...
        printf("** Process %03d: Allocated Frames=%03d PageFaults/References=%03d/%03d\n",
			   cur->pid, cur->pagefault_cnt + 1, cur->pagefault_cnt, cur->reference_cnt);
			   
        pte *cur_pte_lev1 = (pte*)&pas[cur->pid];
        for(int i=0;i<8;i++){
            if(cur_pte_lev1[i].vflag == PAGE_VALID){
                printf("(L1PT) %03d -> %03d\n",i, cur_pte_lev1[i].frame);
                pte *cur_pte_lev2 = (pte*)&pas[cur_pte_lev1[i].frame];
                for(int j=0;j<8;j++){
                    if(cur_pte_lev2[j].vflag == PAGE_VALID){
                        printf("(L2PT) %03d -> %03d REF=%03d\n",
							   i*8+j, cur_pte_lev2[j].frame, cur_pte_lev2[j].ref);
                    }
                }
            }
        }
